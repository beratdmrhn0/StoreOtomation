...
void SAColorMenu::enableNoneColorAction(bool on)
{
    ...
    if (on) {
        if (d_ptr->mNoneColorAction) {
            ...
            connect(d_ptr->mNoneColorAction,
                    &QAction::triggered,
                    this,
...
void SAColorMenu::init(const QList<QColor> &themeCls)
{
    ...
    connect(d_ptr->mCustomColorAction,
            &QAction::triggered,
            this,
            &SAColorMenu::onCustomColorActionTriggered);
    connect(d_ptr->mThemeColorsWidget,
            &SAColorPaletteGridWidget::colorClicked,
            this,
            &SAColorMenu::emitSelectedColor);
    connect(d_ptr->mCustomColorsWidget,
            &SAColorGridWidget::colorClicked,
            this,
            &SAColorMenu::emitSelectedColor);
}
...
public:
    ...
    int mColumnCount{
                     8}; ///< 列数，行数量会根据列数量来匹配,如果设置-1或者0，说明不限定列数量，这样会只有一行
...
{
    if (on) {
        QSpacerItem *horizontalSpacer = new QSpacerItem(40,
                                                        20,
...
{
    connect(d_ptr->mMainColorList,
            &SAColorGridWidget::colorClicked,
            this,
            &SAColorPaletteGridWidget::onMainColorClicked);
    connect(d_ptr->mPaletteColorGrid,
            &SAColorGridWidget::colorClicked,
            this,
                                                                  ...
                                                                  QRect &colorRect)
{
    ...
    if (opt.icon.isNull()) {
        ...
        // 有icon，颜色位于图标下面
        int y = (buttonRect.height() - colorHeight - mSpacing - tmpSize.height())
                / 2; //(ButtonHeight-TotalHeight)/2
        int x = (buttonRect.width() - tmpSize.width()) / 2;
        iconRect = QRect(buttonRect.left() + x,
                         buttonRect.top() + y,
                         tmpSize.width(),
                         tmpSize.height());
    ...
    const QStyleOptionToolButton &opt, QRect &iconRect, QRect &textRect, QRect &colorRect)
{
    ...
    if (opt.icon.isNull()) {
        ...
        iconRect = QRect();
        textRect = QRect(buttonRect.left(),
                         colorRect.bottom() + mSpacing,
                         buttonRect.width(),
                         textHeight);
...
{
    QRect btnRect = q_ptr->style()->subControlRect(QStyle::CC_ToolButton,
                                                   &opt,
...
QRect SAColorToolButton::PrivateData::getIndicatorRect(const QStyleOptionToolButton &opt)
{
    if (opt.features & QStyleOptionToolButton::MenuButtonPopup
        ...
        return q_ptr->style()->subControlRect(QStyle::CC_ToolButton,
                                              &opt,
...
QSize SAColorToolButton::sizeHint() const
{
    ...
    if (Qt::ToolButtonIconOnly == opt.toolButtonStyle
        ...
        w = textSize.width() + d_ptr->mSpacing + iconSize.width() + d_ptr->mMargins.left()
        ...
        w = qMax(textSize.width(), iconSize.width()) + d_ptr->mMargins.left()
            + d_ptr->mMargins.right();
        h = textSize.height() + iconSize.height() + d_ptr->mSpacing + d_ptr->mMargins.top()
            + d_ptr->mMargins.bottom();
    }
...
void SAColorToolButton::paintButton(QStylePainter *p, const QStyleOptionToolButton &opt)
{
    ...
    if (opt.state & QStyle::State_HasFocus) {
        ...
        if (opt.features & QStyleOptionToolButton::MenuButtonPopup) {
            fr.rect.adjust(0,
                           0,
                           style()->pixelMetric(QStyle::PM_MenuButtonIndicator, &opt, this),
                           0);
        }
...
{
public:
    explicit SAPrivateFramelessWidgetData(SAFramelessHelper::PrivateData *pd,
                                          QWidget *pTopLevelWidget);
...
/***** WidgetData *****/
SAPrivateFramelessWidgetData::SAPrivateFramelessWidgetData(SAFramelessHelper::PrivateData *pd,
                                                           QWidget *pTopLevelWidget)
{
    d = pd;
...
bool SAFramelessHelper::eventFilter(QObject *obj, QEvent *event)
{
    ...
    case QEvent::MouseButtonDblClick:
    case QEvent::Leave: {
        SAPrivateFramelessWidgetData *data = d_ptr->m_widgetDataHash.value(
...
void SAFramelessHelper::activateOn(QWidget *topLevelWidget)
{
    if (!d_ptr->m_widgetDataHash.contains(topLevelWidget)) {
        SAPrivateFramelessWidgetData *data = new SAPrivateFramelessWidgetData(d_ptr.get(),
                                                                              topLevelWidget);
...
SARibbonApplicationButton::SARibbonApplicationButton(const QIcon &icon,
                                                     const QString &text,
                                                     QWidget *parent)
    : QToolButton(parent)
{
...
public:
    ...
    // 估算一个最优的文本宽度
    int estimateLargeButtonTextWidth(
        int buttonHeight,
    ...
    default: {
        ...
        if (opt.text.isEmpty()) {
            ...
            if (hasInd) {
                ...
                textRect = buttonRect.adjusted(
                    adjx, 0, 0, 0); // 在buttonRect上变换，因此如果没有图标是不用偏移spacing
            }
        }
        // 最后设置Indicator
        if (hasInd) {
    ...
    if (isEnableWordWrap()) {
        ...
        if (isTextNeedWrap()) {
            ...
            if (hIndicator) {
                ...
                indicatorArrowRect = QRect(textRect.x(),
                                           textRect.y() + dy,
                                           textRect.width(),
                                           indicatorLen);
            }
...
#if SARIBBONTOOLBUTTON_DEBUG_DRAW
            if (trycount > 1) {
                qDebug() << "estimateLargeButtonTextWidth,origin textSize=" << textSize
                         << ",trycount=" << trycount << ",textRect=" << textRect;
            }
#endif
        } while (trycount < 3);
                                   ...
                                   const QRect &textDrawRect)
{
    ...
    style()->drawItemText(&p,
                          ...
                          alignment,
                          label.palette,
                          label.state & QStyle::State_Enabled,
                          text,
                          QPalette::ButtonText);
...
public:
    ...
    SARibbonColorToolButton::ColorStyle mColorStyle{
                                                    SARibbonColorToolButton::ColorUnderIcon}; ///< 颜色显示样式
...
QPixmap SARibbonColorToolButton::PrivateData::createIconPixmap(const QStyleOptionToolButton &opt,
                                                               const QSize &iconsize) const
{
    if (opt.icon.isNull()) { // 没有有图标
        return QPixmap();
                                           ...
                                           bool enableEmit)
{
    ...
    if (d_ptr->mKeyToAction.contains(k)) {
        qWarning() << "key: " << k
                   << " have been exist,you can set key in an unique value when use "
                      "SARibbonActionsManager::registeAction";
        return (false);
    }
...
QVariant SARibbonActionsManagerModel::headerData(int section,
...
void SARibbonActionsManagerModel::setupActionsManager(SARibbonActionsManager *m)
{
    ...
    connect(m,
            &SARibbonActionsManager::actionTagChanged,
            this,
...
void SARibbonQuickAccessBar::addMenu(QMenu *m,
                                     Qt::ToolButtonStyle buttonStyle,
                                     QToolButton::ToolButtonPopupMode popMode)
{
...
void SARibbonPannelLayout::setOptionAction(QAction *action)
{
    ...
    if (action) {
        ...
        if (nullptr == m_optionActionBtn) {
            ...
            QObject::connect(m_optionActionBtn,
                             &SARibbonToolButton::triggered,
                             p,
...
void SARibbonPannelLayout::doLayout()
{
    ...
    if (m_optionActionBtn) {
        ...
        m_optionActionBtn->setIconSize(
            QSize(m_optionActionBtnGeometry.width(), m_optionActionBtnGeometry.height()));
    }
}
...
void SARibbonPannelLayout::updateGeomArray(const QRect &setrect)
{
    ...
    for (int i = 0; i < itemCount; ++i) {
        ...
        if (SARibbonToolButton *tb = qobject_cast<SARibbonToolButton *>(item->widget())) {
            ...
            debug_print__log__
                += QString("| | |-[%1]SARibbonToolButton.sizeHint=(%2,%3),ButtonText=%4\n")
                       .arg(i)
                       .arg(ss__.width())
                       .arg(ss__.height())
                       .arg(tb->text());
    ...
    if (isEnableShowPannelTitle()) {
        m_titleLabelGeometry.setRect(mag.left(),
                                     yTitleBegin,
                                     setrect.width() - mag.left() - mag.right(),
                                     titleH);
    ...
    if (isHaveOptionAction()) {
        ...
        if (isEnableShowPannelTitle()) {
            // 有标题
            m_optionActionBtnGeometry.setRect(m_titleLabelGeometry.right()
    ...
    // 刷新sizeHint
    int heightHint = SARibbonPannel::pannelHeightHint(pannel->fontMetrics(),
                                                      pannel->pannelLayoutMode(),
                                                      titleH);
...
{
#if SA_DEBUG_PRINT_SIZE_HINT
    qDebug() << "SARibbonCategory::PrivateData::updateItemGeometry,categoryName="
             << q_ptr->categoryName();
#endif
    SARibbonCategoryLayout *lay = qobject_cast<SARibbonCategoryLayout *>(q_ptr->layout());
    if (!lay) {
...
// 如果是hide就直接跳过
        ...
        debug_totalSizeHintWidth__
            += QString("|-[%1]pannelSize=(%2,%3),SeparatorSize=(%4,%5),name=(%6) \n")
                   .arg(debug_i__)
    ...
    , d_ptr(new SARibbonCategoryLayout::PrivateData(this))
{
    ...
    connect(d_ptr->mLeftScrollBtn,
            &QToolButton::clicked,
            this,
            &SARibbonCategoryLayout::onLeftScrollButtonClicked);
    connect(d_ptr->mRightScrollBtn,
            &QToolButton::clicked,
            this,
...
// 判断是否超过总长度
    ...
    int x = d_ptr->mXBase;
    if ((categoryAlignment() == SARibbonAlignment::AlignCenter) && (total < categoryWidth)
        && (0 == expandWidth)) {
...
SARibbonGalleryGroupItemDelegate::SARibbonGalleryGroupItemDelegate(SARibbonGalleryGroup *group,
                                                                   QObject *parent)
...
QSize SARibbonGalleryGroupItemDelegate::sizeHint(const QStyleOptionViewItem &option,
                                                 const QModelIndex &index) const
{
    Q_UNUSED(index);
    Q_UNUSED(option);
...
void SARibbonGallery::addGalleryGroup(SARibbonGalleryGroup *group)
{
    ...
    connect(group,
            &SARibbonGalleryGroup::groupTitleChanged,
            this,
...
public:
    ...
    SARibbonPannel::PannelLayoutMode mDefaulePannelLayoutMode{
                                                              SARibbonPannel::ThreeRowMode}; ///< 默认的PannelLayoutMode
    bool mEnableShowPannelTitle{true}; ///< 是否运行pannel的标题栏显示
...
void SARibbonBar::PrivateData::init()
{
    ...
    q_ptr->connect(mApplicationButton,
                   &QAbstractButton::clicked,
                   q_ptr,
...
{
    return calcMainBarHeight(tabBarHeigth(),
                             titleBarHeight(),
                             categoryHeight(),
                             mIsTabOnTitle,
...
void SARibbonBar::PrivateData::resetSize()
{
    ...
    int mainBarHeight = calcMainBarHeight(th,
                                          titleBarHeight(),
                                          categoryHeight(),
                                          mIsTabOnTitle,
                                          mCurrentRibbonMode);
...
void SARibbonBar::addContextCategory(SARibbonContextCategory *context)
{
    ...
    connect(context,
            &SARibbonContextCategory::categoryPageAdded,
            this,
            &SARibbonBar::onContextsCategoryPageAdded);
    connect(context,
            &SARibbonContextCategory::categoryTitleChanged,
            this,
...
void SARibbonBar::showMinimumModeButton(bool isShow)
{
    if (isShow) {
        ...
        d_ptr->mMinimumCategoryButtonAction->setIcon(
            style()->standardIcon(isMinimumMode() ? QStyle::SP_TitleBarUnshadeButton
                ...
                if (d_ptr->mStackedContainerWidget->isPopupMode()) {
                    ...
                    if (!d_ptr->mStackedContainerWidget->rect().contains(mouseEvent->pos())) {
                        ...
                        if (clickedWidget == d_ptr->mRibbonTabBar) {
                            const QPoint targetPoint = clickedWidget->mapFromGlobal(
...
void SARibbonBar::resizeInLooseStyle()
{
    ...
    if (d_ptr->mQuickAccessBar) {
        if (d_ptr->mQuickAccessBar->isVisibleTo(this)) {
            ...
            // 上下留1px的边线
            d_ptr->mQuickAccessBar->setGeometry(x,
                                                y + 1,
                                                quickAccessBarSize.width(),
                                                validTitleBarHeight);
        }
    }
    ...
    if (d_ptr->mApplicationButton) {
        if (d_ptr->mApplicationButton->isVisibleTo(this)) {
            d_ptr->mApplicationButton->setGeometry(x,
                                                   y,
                                                   d_ptr->mApplicationButton->sizeHint().width(),
                                                   tabH);
...
void SARibbonBar::resizeInCompactStyle()
{
    ...
    if (d_ptr->mQuickAccessBar) {
        if (d_ptr->mQuickAccessBar->isVisibleTo(this)) {
            ...
            // 上下留1px的边线
            d_ptr->mQuickAccessBar->setGeometry(endX,
                                                y + 1,
                                                quickAccessBarSize.width(),
                                                validTitleBarHeight - 2);
        }
    }
    // cornerWidget - TopLeftCorner
    if (QWidget *connerL = cornerWidget(Qt::TopLeftCorner)) {
 ...
 */
SARibbonPannelOptionButton *SARibbonElementFactory::createRibbonPannelOptionButton(
    SARibbonPannel *pannel)
{
...
SARibbonCustomizeData SARibbonCustomizeData::makeAddPannelCustomizeData(
    const QString &title, int index, const QString &categoryobjName, const QString &objName)
{
...
SARibbonCustomizeData SARibbonCustomizeData::makeRenamePannelCustomizeData(
    const QString &newname, const QString &categoryobjName, const QString &pannelObjName)
{
 ...
 * @return
 */
SARibbonCustomizeData SARibbonCustomizeData::makeChangeCategoryOrderCustomizeData(
...
SARibbonCustomizeData SARibbonCustomizeData::makeChangePannelOrderCustomizeData(
    const QString &categoryobjName, const QString &pannelObjName, int moveindex)
{
...
QList<SARibbonCustomizeData> remove_indexs(const QList<SARibbonCustomizeData> &csd,
...
QList<SARibbonCustomizeData> sa_customize_datas_from_xml(QXmlStreamReader *xml,
                                                         SARibbonActionsManager *mgr)
{
...
bool sa_apply_customize_from_xml_file(const QString &filePath,
                                      SARibbonBar *bar,
                                      SARibbonActionsManager *mgr)
{
    QFile f(filePath);
...
public:
    ...
    void retranslateUi(QWidget *customizeWidget)
    {
        ...
        labelSelectAction->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                           "Please Select",
                                                           Q_NULLPTR)); // cn:请选择
        lineEditSearchAction->setInputMask(QString());
        lineEditSearchAction->setText(QString());
        lineEditSearchAction->setPlaceholderText(QApplication::translate("SARibbonCustomizeWidget",
                                                                         "Find Command",
                                                                         Q_NULLPTR)); // cn:查找命令
        pushButtonAdd->setText(
            QApplication::translate("SARibbonCustomizeWidget", "Add >>", Q_NULLPTR)); // cn:添加 >>
        pushButtonDelete->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                          "<< Remove",
                                                          Q_NULLPTR)); // cn:<< 移除
        labelCustomize->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                        "Customize the Ribbon",
                                                        Q_NULLPTR)); // cn:自定义功能区
        radioButtonMainCategory->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                                 "Main Category",
                                                                 Q_NULLPTR)); // cn:主选项卡
        radioButtonAllCategory->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                                "All Category",
                                                                Q_NULLPTR)); // cn:所有选项卡
        pushButtonNewCategory->setText(QApplication::translate("SARibbonCustomizeWidget",
                                                               "New Category",
                                                               Q_NULLPTR)); // cn:新建选项卡
        ...
        pushButtonRename->setText(
            QApplication::translate("SARibbonCustomizeWidget", "Rename", Q_NULLPTR)); // cn:重命名
        pushButtonReset->setText(
            QApplication::translate("SARibbonCustomizeWidget", "reset", Q_NULLPTR)); // cn:重置
...
public:
    SARibbonCustomizeWidget::RibbonTreeShowType mShowType{
                                                          SARibbonCustomizeWidget::ShowAllCategory};     ///< 显示类型
...
void SARibbonCustomizeWidget::PrivateData::updateModel()
{
    ...
    for (const SARibbonCategory *c : qAsConst(categorys)) {
        ...
        for (const SARibbonPannel *p : qAsConst(pannels)) {
            ...
            if (p->isCanCustomize()) {
                pi->setData(true,
                            SARibbonCustomizeWidget::CanCustomizeRole); // 标记这个是可以自定义的
            }
            ci->appendRow(pi);
...
SARibbonCustomizeWidget::SARibbonCustomizeWidget(SARibbonBar *ribbonbar,
                                                 QWidget *parent,
                                                 Qt::WindowFlags f)
    : QWidget(parent, f)
    , d_ptr(new SARibbonCustomizeWidget::PrivateData(this))
...
void SARibbonCustomizeWidget::initConnection()
{
    ...
    connect(d_ptr->mRibbonModel,
            &QStandardItemModel::itemChanged,
            this,
            &SARibbonCustomizeWidget::onItemChanged);
    connect(ui->lineEditSearchAction,
...
void SARibbonCustomizeWidget::onPushButtonNewCategoryClicked()
{
    ...
    QStandardItem *ni = new QStandardItem(
        tr("new category[customize]%1").arg(++(d_ptr->mCustomizeCategoryCount)));
    ...
    ni->setData(true,
                SARibbonCustomizeWidget::CanCustomizeRole); // 有CustomizeRole，必有CanCustomizeRole
...
void SARibbonCustomizeWidget::onPushButtonRenameClicked()
{
    ...
    if (0 == level) {
        ...
        SARibbonCustomizeData d
            = SARibbonCustomizeData::makeRenameCategoryCustomizeData(text, cateObjName);
        d_ptr->mCustomizeDatasCache.append(d);
    } else if (1 == level) {
...
void SARibbonCustomizeWidget::onPushButtonDeleteClicked()
{
    ...
    if (0 == level) {
        // 删除category
        SARibbonCustomizeData d = SARibbonCustomizeData::makeRemoveCategoryCustomizeData(
        ...
        QString pannelObjName = d_ptr->itemObjectName(item);
        SARibbonCustomizeData d
            = SARibbonCustomizeData::makeRemovePannelCustomizeData(catObjName, pannelObjName);
        d_ptr->mCustomizeDatasCache.append(d);
    } else if (2 == level) {
        // 删除Action
        ...
        SARibbonCustomizeData d
            = SARibbonCustomizeData::makeRemoveActionCustomizeData(catObjName,
                                                                   pannelObjName,
                                                                   key,
                                                                   d_ptr->mActionMgr);
        d_ptr->mCustomizeDatasCache.append(d);
    ...
    removeItem(item);
    // 删除后重新识别
    ui->pushButtonAdd->setEnabled(selectedAction() && isSelectedItemIsCustomize()
                                  && selectedRibbonLevel() > 0);
...
void SARibbonCustomizeWidget::onTreeViewResultClicked(const QModelIndex &index)
{
    ...
    ui->pushButtonRename->setEnabled(
        level != 2
        || isItemCanCustomize(item)); // QAction 不能改名 ， 有CustomizeRole，必有CanCustomizeRole
}
...
void SARibbonCustomizeWidget::onToolButtonUpClicked()
{
    ...
    if (0 == level) {
        // 移动category
        SARibbonCustomizeData d
            = SARibbonCustomizeData::makeChangeCategoryOrderCustomizeData(d_ptr->itemObjectName(
                                                                              item),
                                                                          -1);
        d_ptr->mCustomizeDatasCache.append(d);
        int r = item->row();
...
void SARibbonCustomizeWidget::onItemChanged(QStandardItem *item)
{
    ...
    if (0 == level) {
        if (item->isCheckable()) {
            ...
            SARibbonCustomizeData d
                = SARibbonCustomizeData::makeVisibleCategoryCustomizeData(objname,
                                                                          item->checkState()
                                                                              == Qt::Checked);
            d_ptr->mCustomizeDatasCache.append(d);
        }
...
public:
    ...
    void retranslateUi(QWidget *customizeDialog)
    {
        ...
        pushButtonCancel->setText(
            QApplication::translate("SARibbonCustomizeDialog", "Cancel", Q_NULLPTR));
...
SARibbonCustomizeDialog::SARibbonCustomizeDialog(SARibbonMainWindow *ribbonWindow,
                                                 QWidget *p,
                                                 Qt::WindowFlags f)
    : QDialog(p, f)
...
public:
    SARibbonMainWindow::RibbonTheme mCurrentRibbonTheme{
                                                        SARibbonMainWindow::RibbonThemeOffice2021Blue};
    ...
    if (SARibbonBar *bar = ribbonBar()) {
        ...
        case RibbonThemeOffice2016Blue:
            bar->setContextCategoryColorList(
                QList<QColor>() << QColor(18, 64, 120)); //< 设置空颜色列表会重置为默认色系
            break;
        case RibbonThemeOffice2021Blue:
            bar->setContextCategoryColorList(
                QList<QColor>() << QColor(0, 0, 0, 0)); //< 设置空颜色列表会重置为默认色系
            break;
        default:
            break;
        }
...
void SARibbonMainWindow::resizeEvent(QResizeEvent *e)
{
    ...
    if (wg) {
        ...
        wg->setGeometry(frameGeometry().width() - wgSizeHint.width(),
                        0,
                        wgSizeHint.width(),
                        wgSizeHint.height());
    }
